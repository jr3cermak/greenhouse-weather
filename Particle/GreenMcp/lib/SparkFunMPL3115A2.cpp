/*
 ...
 This library allows an Arduino to read from the MPL3115A2 low-cost
 high-precision pressure sensor.
 
 If you have feature suggestions or need support please use the github
 support page: https://github.com/sparkfun/MPL3115A2_Breakout

 Hardware Setup: The MPL3115A2 lives on the I2C bus. Attach the SDA
 pin to A4, SCL to A5. Use inline 10k resistors if you have a 5V board.
 If you are using the SparkFun breakout board you *do not* need 4.7k
 pull-up resistors on the bus (they are built-in).
 
 Link to the breakout board product:
 
 Software:
 .assign() Sets the I2C communcation channel to be used.
 ...
 Date: June 7, 2017
 Revise to contact as a slave from a master, do not reinitialize as
 master.
...
//Begin
/*******************************************************************************************/
// Define I2C communications link
void MPL3115A2::assign(TwoWire &wirePort)
{
...
{
  // Only use this function if we are to be the master
  _i2cPort->begin();
}
...
float MPL3115A2::readAltitude()
{
	...
	_i2cPort->beginTransmission(MPL3115A2_ADDRESS);
	...
	if (_i2cPort->requestFrom(MPL3115A2_ADDRESS, 3) != 3) { // Request three bytes
		return -999;
	}
...
float MPL3115A2::readTemp()
{
	...
	_i2cPort->endTransmission(false); // Send data to I2C dev with option for a repeated start. THIS IS NECESSARY and not supported before Arduino V1.0.1!
	if (_i2cPort->requestFrom(MPL3115A2_ADDRESS, 2) != 2) { // Request two bytes
		return -999;
	}
...
byte MPL3115A2::IIC_Read(byte regAddr)
{
  ...
  _i2cPort->endTransmission(false); // Send data to I2C dev with option for a repeated start. THIS IS NECESSARY and not supported before Arduino V1.0.1!
  _i2cPort->requestFrom(MPL3115A2_ADDRESS, 1); // Request the data...
